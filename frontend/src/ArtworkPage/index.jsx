    ...
    <main className="bg-gray-900 min-h-screen">
      ...
      <div className="container mx-auto px-5">
        ...
        <div className="flex items-center justify-around gap-x-10 py-10">
          {otherArtsArray.map(artwork => { return(
            <Link to={`/artwork/${artwork.id}`} className={"hover:bg-gray-800"}>
              <img style={{maxWidth: "20em"}} className="mb-5 shadow-xl mx-auto"
                src={artwork.image}
                alt={artwork.name}
              />
              <div className="pl-3">
                <h2 className="dark:text-white text-lg font-semibold mb-1">{artwork.name}</h2>
                <p className="dark:text-gray-300 text-sm mb-3">{artwork.description}</p>
              </div>
            </Link>
            );
          })}
